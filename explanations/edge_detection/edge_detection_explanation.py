...
class EdgeDetectionExplainer(object):
  """docstring for EdgeExplainer"""
  def __init__(self, model):
    super(EdgeDetectionExplainer, self).__init__()
    self.model = model
  ...
  def EdgeDetectionAttributionToImage(self, innvestigate_attribution):
    img = np.sum(innvestigate_attribution, axis=2)
    return img*255

  def GenerateEdgeDetectionExplanationImage(self,input_image,explanation_values):
