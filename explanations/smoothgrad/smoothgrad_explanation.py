...
class SmoothGradExplainer(InnvestigateExplainer):
  """docstring for SmoothGradExplainer"""
  def __init__(self, model):
    super(SmoothGradExplainer, self).__init__(model, explanation_method="smoothgrad")
  
  def SmoothGradAttributionToImage(self, smoothgrad_attribution):
    return super(SmoothGradExplainer, self).InnvestigateAttributionToImage(smoothgrad_attribution)

  def GenerateSmoothGradExplanationImage(self,input_image,explanation_values):
      return super(SmoothGradExplainer, self).GenerateInnvestigateExplanationImage(input_image,explanation_values)

    
  def Explain(self,input_image, additional_args = {}):
    return super(SmoothGradExplainer, self).Explain(input_image, additional_args=additional_args)
