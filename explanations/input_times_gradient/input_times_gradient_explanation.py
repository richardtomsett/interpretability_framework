...
class InputTimesGradientExplainer(InnvestigateExplainer):
  """docstring for InputTimesGradientExplainer"""
  def __init__(self, model):
    super(InputTimesGradientExplainer, self).__init__(model, explanation_method="input_t_gradient")
  
  def InputTimesGradientAttributionToImage(self, input_t_gradient_attribution):
    return super(InputTimesGradientExplainer, self).InnvestigateAttributionToImage(input_t_gradient_attribution)

  def GenerateInputTimesGradientExplanationImage(self,input_image,explanation_values):
      return super(InputTimesGradientExplainer, self).GenerateInnvestigateExplanationImage(input_image,explanation_values)

    
  def Explain(self,input_image, additional_args = {}):
    return super(InputTimesGradientExplainer, self).Explain(input_image, additional_args=additional_args)
